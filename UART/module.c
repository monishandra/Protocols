...
static enum { PARITY_NONE, PARITY_ODD, PARITY_EVEN } parity_setting = PARITY_NONE;
...
/*static void softUart_set_termios(struct uart_port *port, struct ktermios *new, struct ktermios *old)
{
    ...
    {
    baud=2400;
     printk(KERN_INFO "before call %d",baud);
...
static void softUart_set_termios(struct uart_port *port, struct ktermios *new, struct ktermios *old)
{
    ...
    // Add parity setting here
    if (new->c_cflag & PARENB)
    {
        if (new->c_cflag & PARODD)
        {
            parity_setting = PARITY_ODD;
            printk(KERN_INFO "Odd parity enabled\n");
        }
        else
        {
            parity_setting = PARITY_EVEN;
            printk(KERN_INFO "Even parity enabled\n");
        }
    }
    else
    {
        parity_setting = PARITY_NONE;
        printk(KERN_INFO "No parity enabled\n");
    }
}
